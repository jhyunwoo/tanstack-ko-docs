...
- 기본적으로 Query 결과는 **데이터가 실제로 변경되었는지 감지하기 위해 구조적으로 공유됩니다**. 그렇지 않은 경우 **데이터 참조는 변경되지 않은 상태로 유지**되어 값 안정화에 더 도움이 됩니다. 이 개념이 낯설게 들린다면 걱정하지 마세요! 99.9%의 경우 이 기능을 비활성화할 필요가 없으며 이를 통해 무료로 앱 성능을 높일 수 있습니다.
