...
queries와 달리 mutations는 ​​일반적으로 데이터 생성/업데이트/삭제 또는 서버 부작용을 수행하는 데 사용됩니다. 이를 위해 TanStack Query는 `useMutation` 기능을 내보냅니다.
...
`persistQueryClient` 플러그인을 사용하여 오프라인 mutations를 유지하는 경우 기본 mutation 기능을 제공하지 않으면 페이지를 다시 로드할 때 mutations를 재개할 수 없습니다.
...
mutations에 대한 자세한 내용은 [TanStack Query에서 Mutations 마스터링에 대한 TkDodo의 기사](https://tkdodo.eu/blog/mastering-mutations-in-react-query)를 참조하세요.
