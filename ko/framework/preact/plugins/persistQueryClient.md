...
- [createSyncStoragePersister](../../react/plugins/createSyncStoragePersister.md)
- [createAsyncStoragePersister](../../react/plugins/createAsyncStoragePersister.md)
...
- 귀하의 query/mutation는 [hydration](../../react/reference/hydration.md#dehydrate)이며 귀하가 제공한 지속기에 의해 저장됩니다.
...
- 이전에 지속된 디하이드레이션된 query/mutation 캐시를 지속기에서 전달된 query의 query 캐시로 다시 [hydration](../../react/reference/hydration.md#hydrate)하려고 시도합니다. 클라이언트.
...
또한 Preact 구성 요소 수명주기 외부의 변경 사항을 구독하는 경우 구독을 취소할 방법이 없습니다.
...
// 🚨 복원과 동시에 발생
PreactDOM.createRoot(rootElement).render(App /)
```
### PersistQueryClientProvider

이 사용 사례에서는 `PersistQueryClientProvider`를 사용할 수 있습니다. 이는 Preact 구성 요소 수명 주기에 따라 올바르게 구독/구독 취소를 확인하고 복원하는 동안 queries가 가져오기를 시작하지 않도록 합니다. Queries는 계속 렌더링되지만 데이터가 복원될 때까지 `fetchingState: 'idle'`에 저장됩니다. 그런 다음 복원된 데이터가 충분히 _신선_하지 않으면 다시 가져오며 _initialData_도 존중됩니다. 일반 [QueryClientProvider](../../react/reference/QueryClientProvider.md) _대신_ 사용할 수 있습니다.
...
PreactDOM.createRoot(rootElement).render(
  <PersistQueryClientProvider
    client={queryClient}
...
`PersistQueryClientProvider`는 [QueryClientProvider](../../react/reference/QueryClientProvider.md)와 동일한 소품을 사용하며 추가로 다음을 수행합니다.
