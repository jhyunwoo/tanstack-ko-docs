...
Preact Query는 이제 **TypeScript**로 작성되어 라이브러리와 프로젝트가 유형에 안전한지 확인합니다!
...
- **preact-query 패키지 버전을 특정 패치 릴리스로 잠그고 모든 릴리스 간에 유형이 수정되거나 업그레이드될 수 있다는 예상으로 업그레이드하는 것이 좋습니다**
- Preact Query의 유형과 관련되지 않은 공개 API는 여전히 semver를 매우 엄격하게 따릅니다.

## 유형 추론

Preact Query의 유형은 일반적으로 매우 잘 흐르므로 직접 유형 주석을 제공할 필요가 없습니다.
...
Preact Query는 `status` 필드와 파생된 상태 boolean 플래그로 구별되는 query 결과에 [차별화된 공용체 유형](https://www.typescriptlang.org/docs/handbook/typescript-in-5- Minutes-func.html#차별화된-unions)을 사용합니다. 이를 통해 예를 들어 확인할 수 있습니다. `data`를 정의하기 위한 `success` 상태:
...
[전역 오류 유형](#registering-a-global-error)을 등록하는 것과 마찬가지로 전역 `Meta` 유형을 등록할 수도 있습니다. 이렇게 하면 [useQuery](../react/reference/useQuery.md) 및 [useMutation](../react/reference/useMutation.md)의 선택적 `meta` 필드가 일관되게 유지되고 유형이 안전해집니다. 등록된 유형은 `meta`가 객체로 유지되도록 `Record<string, unknown>`를 확장해야 합니다.
...
유형 추론에 대한 팁과 요령은 [Preact Query 및 TypeScript](https://tkdodo.eu/blog/preact-query-and-type-script) 문서를 참조하세요. 가능한 최상의 유형 안전성을 얻는 방법을 알아보려면 [Type-safe Preact Query](https://tkdodo.eu/blog/type-safe-preact-query)를 읽어보세요. [Query 옵션 API](https://tkdodo.eu/blog/the-query-options-api)에서는 `queryOptions` 도우미 함수와 함께 유형 추론이 작동하는 방식을 간략하게 설명합니다.
