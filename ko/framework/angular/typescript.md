...
TanStack Query는 이제 **TypeScript**로 작성되어 라이브러리와 프로젝트가 유형에 안전한지 확인합니다!
...
- **angular-query-실험용 패키지 버전을 특정 패치 릴리스로 잠그고 모든 릴리스 간에 유형이 수정되거나 업그레이드될 수 있다는 예상으로 업그레이드하는 것이 좋습니다**
- TanStack Query의 유형과 관련되지 않은 공개 API 및 - 실험 단계 이후에도 angle-query 패키지는 여전히 semver를 매우 엄격하게 따릅니다.

## 유형 추론

TanStack Query의 유형은 일반적으로 매우 잘 흐르므로 직접 유형 주석을 제공할 필요가 없습니다.
...
이는 `queryFn`에 잘 정의된 반환 유형이 있는 경우 가장 잘 작동합니다. 대부분의 데이터 가져오기 라이브러리는 기본적으로 `any`를 반환하므로 올바른 형식의 함수로 추출해야 합니다.



이 예에서는 Group[]을 HttpClient의 `get` 메서드의 유형 매개 변수에 전달합니다.
...
TanStack Query는 `status` 필드와 파생된 상태 boolean 플래그로 구별되는 query 결과에 [차별화된 공용체 유형](https://www.typescriptlang.org/docs/handbook/typescript-in-5- Minutes-func.html#차별화된-unions)을 사용합니다. 이를 통해 예를 들어 확인할 수 있습니다. `data`를 정의하기 위한 `isSuccess()` 상태:
...
```
> TypeScript는 현재 객체 메서드에 대한 구별된 공용체를 지원하지 않습니다. query 결과와 같은 객체의 신호 필드 범위를 좁히는 것은 boolean을 반환하는 신호에서만 작동합니다. `status() === 'success'`보다 `isSuccess()` 및 유사한 boolean 상태 신호를 사용하는 것이 좋습니다.
...
그러나 이는 `injectQuery`의 다른 모든 제네릭에 대한 유형 추론이 더 이상 작동하지 않는다는 단점이 있습니다. 일반적으로 `Error`가 아닌 것을 던지는 것은 좋은 습관으로 간주되지 않습니다. 따라서 `AxiosError`와 같은 하위 클래스가 있는 경우 _유형 축소_를 사용하여 오류 필드를 더 구체적으로 만들 수 있습니다.
...
[전역 오류 유형](#registering-a-global-error)을 등록하는 것과 마찬가지로 전역 `Meta` 유형을 등록할 수도 있습니다. 이렇게 하면 [injectQuery](./reference/functions/injectQuery.md) 및 [injectMutation](./reference/functions/injectMutation.md)의 선택적 `meta` 필드가 일관되게 유지되고 유형이 안전해집니다. 등록된 유형은 `meta`가 객체로 유지되도록 `Record<string, unknown>`를 확장해야 합니다.
...
query 옵션을 `injectQuery`에 인라인하면 자동 유형 추론이 수행됩니다. 그러나 query 옵션을 별도의 함수로 추출하여 `injectQuery`와 예를 들어 다음과 같이 공유할 수 있습니다. `prefetchQuery` 또는 서비스에서 관리하세요. 이 경우 유형 추론이 손실됩니다. 다시 가져오려면 `queryOptions` 도우미를 사용할 수 있습니다.
...
export class Component {
  ...
  // query 옵션을 반환하는 신호를 전달할 수도 있습니다.
...
// ^? 데이터: 게시 | 한정되지 않은
```
`queryOptions`가 없으면 타입 매개변수를 전달하지 않는 한 데이터 유형을 알 수 없습니다.
