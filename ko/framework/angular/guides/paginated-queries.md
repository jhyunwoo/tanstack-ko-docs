...
query에 대한 pageIndex(또는 커서)를 이상적으로 증가시키려는 다음 예를 고려하십시오. `injectQuery`를 사용한다면 **기술적으로는 여전히 잘 작동하겠지만** 각 페이지 또는 커서에 대해 서로 다른 queries가 생성되고 삭제됨에 따라 UI가 `success` 및 `pending` 상태에 들어오고 나갈 것입니다. `placeholderData`를 `(previousData) => previousData` 또는 TanStack Query에서 내보낸 `keepPreviousData` 함수로 설정하면 몇 가지 새로운 기능을 얻을 수 있습니다.
  ...
  template: `
    <div>
      ...
      @if (query.status() === 'pending') {
        ...
        <!-- 'data'는 최신 페이지의 데이터로 해석됩니다. -->
        <!-- 또는 새 페이지를 가져오는 경우 마지막으로 성공한 페이지의 데이터 -->
        <div>
      ...
      <!-- 마지막 페이지의 데이터가 페이지 요청 사이에 남아 있을 가능성이 있으므로 -->
      <!-- 'isFetching'을 사용하여 백그라운드 로딩을 ​​표시할 수 있습니다. -->
      <!-- 상태 === '보류 중' 상태가 트리거되지 않으므로 표시기 -->
      @if (query.isFetching()) {
        <span> Loading...</span>
      }
    </div>
...
흔하지는 않지만 `placeholderData` 옵션은 `injectInfiniteQuery` 기능과 완벽하게 작동하므로 시간이 지남에 따라 무한한 query 키가 변경되는 동안 사용자가 캐시된 데이터를 계속해서 볼 수 있도록 원활하게 허용할 수 있습니다.