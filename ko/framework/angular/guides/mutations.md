...
queries와 달리 mutations는 ​​일반적으로 데이터 생성/업데이트/삭제 또는 서버 부작용을 수행하는 데 사용됩니다. 이를 위해 TanStack Query는 `injectMutation` 기능을 내보냅니다.
...
`injectMutation`에는 mutation 수명 주기 동안 모든 단계에서 빠르고 쉽게 부작용을 허용하는 몇 가지 도우미 옵션이 함께 제공됩니다. 이는 [mutations 이후 queries 무효화 및 다시 가져오기](../../react/guides/invalidations-from-mutations.md) 및 심지어 [낙관적 업데이트](../../react/guides/optimistic-updates.md)에도 유용합니다.
...
`mutate`를 호출할 때 `injectMutation`에 정의된 것 이상으로 **추가 콜백을 트리거**하고 싶을 수도 있습니다. 이는 구성요소별 부작용을 유발하는 데 사용될 수 있습니다. 그렇게 하려면 mutation 변수 다음에 `mutate` 함수에 동일한 콜백 옵션을 제공할 수 있습니다. 지원되는 옵션에는 `onSuccess`, `onError` 및 `onSettled`가 포함됩니다. mutation가 완료되기 _전에_ 구성 요소가 삭제되면 이러한 추가 콜백이 실행되지 않는다는 점을 명심하세요.
...
연속적인 mutations의 경우 `onSuccess`, `onError` 및 `onSettled` 콜백 처리에 약간의 차이가 있습니다. `mutate` 함수에 전달되면 구성 요소가 여전히 활성 상태인 경우에만 _한 번만_ 실행됩니다. 이는 `mutate` 함수가 호출될 때마다 mutation 관찰자가 제거되고 다시 구독된다는 사실 때문입니다. 반대로 `injectMutation` 핸들러는 각 `mutate` 호출에 대해 실행됩니다.

> `injectMutation`에 전달된 `mutationFn`는 비동기식일 가능성이 높습니다. 이 경우 mutations가 이행되는 순서는 `mutate` 함수 호출 순서와 다를 수 있습니다.
...
이는 기술적 한계입니다. 외부 저장소에 지속되면 함수를 직렬화할 수 없으므로 mutations의 상태만 지속됩니다. hydration 이후에는 mutation를 트리거하는 구성 요소가 초기화되지 않을 수 있으므로 `resumePausedMutations`를 호출하면 `No mutationFn found` 오류가 발생할 수 있습니다.
