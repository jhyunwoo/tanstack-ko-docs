...
기존 데이터 집합에 추가로 "더 많은 데이터를 로드"하거나 "무한 스크롤"할 수 있는 렌더링 목록도 매우 일반적인 UI 패턴입니다. TanStack Query는 이러한 유형의 목록을 쿼리하기 위해 `injectInfiniteQuery`라는 유용한 `injectQuery` 버전을 지원합니다.

`injectInfiniteQuery`를 사용하면 몇 가지 다른 점을 알 수 있습니다.
...
- 기본적으로 `injectInfiniteQuery`가 첫 번째 데이터 그룹을 요청하기를 기다리는 중입니다.
