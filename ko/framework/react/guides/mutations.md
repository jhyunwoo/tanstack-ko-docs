...
> 중요: `mutate` 함수는 비동기 함수이므로 **React 16 및 이전 버전**의 이벤트 콜백에서 직접 사용할 수 없습니다. `onSubmit`의 이벤트에 액세스해야 하는 경우 `mutate`를 다른 함수로 래핑해야 합니다. 이는 [React 이벤트 풀링](https://reactjs.org/docs/legacy-event-pooling.html) 때문입니다.

[//]: # 'Info1'
[//]: # 'Example2'

```tsx
// 이는 React 16 및 이전 버전에서는 작동하지 않습니다.
const CreateTodo = () => {
  const mutation = useMutation({
    mutationFn: (event) => {
...
`useMutation`에는 mutation 수명 주기 동안 모든 단계에서 빠르고 쉽게 부작용을 허용하는 몇 가지 도우미 옵션이 함께 제공됩니다. 이는 [mutations 이후 queries 무효화 및 다시 가져오기](invalidations-from-mutations.md) 및 심지어 [낙관적 업데이트](optimistic-updates.md)에도 유용합니다.
...
[persistQueryClient](../plugins/persistQueryClient.md)을 사용하여 오프라인 mutations를 유지하는 경우 기본 mutation 기능을 제공하지 않으면 페이지를 다시 로드할 때 mutations를 재개할 수 없습니다.
...
또한 queries 및 mutations를 모두 다루는 광범위한 [오프라인 예시](../examples/offline.md)도 있습니다.
...
mutations에 대한 자세한 내용은 [React Query에서 Mutations 마스터링에 대한 TkDodo의 기사](https://tkdodo.eu/blog/mastering-mutations-in-react-query)를 참조하세요.
