...
> suspense 모드에서 React Query를 사용하는 경우 첫 번째 query가 내부적으로 Promise을 발생시키고 다른 queries가 실행되기 전에 구성 요소를 일시 중단하기 때문에 이 병렬 처리 패턴이 작동하지 않습니다. 이 문제를 해결하려면 `useSuspenseQueries` 후크(권장됨)를 사용하거나 각 `useSuspenseQuery` 인스턴스에 대해 별도의 구성 요소를 사용하여 자체 병렬 처리를 조정해야 합니다.
