...
query는 **고유 키**에 연결된 비동기 데이터 소스에 대한 선언적 종속성입니다. query는 Promise 기반 방법(GET 및 POST 방법 포함)과 함께 사용하여 서버에서 데이터를 가져올 수 있습니다. 귀하의 방법이 서버의 데이터를 수정하는 경우 [Mutations](mutations.md)를 대신 사용하는 것이 좋습니다.

[//]: # 'SubscribeDescription'

구성요소 또는 사용자 정의 후크에서 query를 구독하려면 최소한 다음을 사용하여 `useQuery` 후크를 호출하세요.
...
`useQuery`에서 반환된 query 결과에는 템플릿 작성 및 기타 데이터 사용에 필요한 query에 대한 모든 정보가 포함되어 있습니다.
...
- `fetchStatus === 'paused'` - query를 가져오려고 했으나 일시 중지되었습니다. 이에 대한 자세한 내용은 [네트워크 모드](network-mode.md) 가이드를 참조하세요.
...
상태 확인을 수행하는 다른 방법은 [TkDodo의 이 기사](https://tkdodo.eu/blog/status-checks-in-react-query)를 참조하세요.
