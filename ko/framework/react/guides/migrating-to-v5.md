...
보다 현대적이고 성능이 뛰어나며 작은 번들을 생성하기 위해 브라우저 목록을 업데이트했습니다. 요구 사항은 [여기](../installation.md#requirements)에서 확인할 수 있습니다.
...
```
다른 종류의 오류를 전역적으로 설정하는 방법은 [TypeScript](../typescript.md#registering-a-global-error)를 참조하세요.
...
React Query v5에는 React 18.0 이상이 필요합니다. 이는 React 18.0 이상에서만 사용할 수 있는 새로운 `useSyncExternalStore` 후크를 사용하고 있기 때문입니다. 이전에는 React에서 제공하는 shim을 사용해 왔습니다.

### `contextSharing` 소품이 QueryClientProvider에서 제거되었습니다.

이전에는 `contextSharing` 속성을 사용하여 창 전체에서 query 클라이언트 컨텍스트의 첫 번째(및 최소한 하나) 인스턴스를 공유할 수 있었습니다. 이를 통해 TanStack Query가 다양한 번들 또는 마이크로프론트엔드에서 사용된 경우 모듈 범위에 관계없이 모두 동일한 컨텍스트 인스턴스를 사용하게 됩니다.

v5에서 사용자 정의 컨텍스트 소품 제거에 대해서는 [사용자 정의 queryClient 인스턴스를 위해 제거된 사용자 정의 컨텍스트 소품](#removed-custom-context-prop-in-favor-of-custom-queryclient-instance) 섹션을 참조하세요. 애플리케이션의 여러 패키지에서 동일한 query 클라이언트를 공유하려는 경우 공유 사용자 정의 `queryClient` 인스턴스를 직접 전달할 수 있습니다.

### 더 이상 React 및 React Native에서 `unstable_batchedUpdates`를 일괄 처리 기능으로 사용하지 않습니다.

`unstable_batchedUpdates` 함수는 React 18에서 noop이므로 더 이상 `react-query`에서 일괄 처리 함수로 자동 설정되지 않습니다.

프레임워크가 사용자 정의 일괄 처리 기능을 지원하는 경우 `notifyManager.setBatchNotifyFunction`를 호출하여 TanStack Query에 이를 알릴 수 있습니다.

예를 들어 `solid-query`에서 배치 기능을 설정하는 방법은 다음과 같습니다.
...
동시 기능 및 전환을 더 효과적으로 지원하기 위해 hydration API를 일부 변경했습니다. `Hydrate` 구성 요소의 이름이 `HydrationBoundary`로 바뀌었고 `useHydrate` 후크가 제거되었습니다.

`HydrationBoundary`는 더 이상 mutations를 수화하지 않고 queries만 수화합니다. mutations를 수화하려면 낮은 수준의 `hydrate` API 또는 `persistQueryClient` 플러그인을 사용하세요.

마지막으로 기술적인 세부 사항으로 queries가 수화되는 시점이 약간 변경되었습니다. 새로운 queries는 렌더링 단계에서 여전히 하이드레이션되어 SSR이 평소대로 작동하지만 캐시에 이미 존재하는 모든 queries는 이제 대신 효과로 하이드레이션됩니다(해당 데이터가 캐시에 있는 것보다 최신인 경우). 일반적으로 응용 프로그램 시작 시 한 번만 수분을 공급하는 경우에는 영향을 미치지 않지만 서버 구성 요소를 사용하고 페이지 탐색에서 hydration에 대한 새로운 데이터를 전달하는 경우 페이지가 즉시 다시 렌더링되기 전에 이전 데이터가 깜박이는 것을 볼 수 있습니다.

이 마지막 변경 사항은 기술적으로 심각한 변경 사항이며 페이지 전환이 완전히 커밋되기 전에 _기존_ 페이지의 콘텐츠를 조기에 업데이트하지 않도록 만들어졌습니다. 귀하가 취해야 할 조치는 없습니다.

```tsx
- import { Hydrate } from '@tanstack/react-query' // [!코드-]
+ import { HydrationBoundary } from '@tanstack/react-query' // [!코드 ++]


- <Hydrate state={dehydratedState}> // [!코드-]
+ <HydrationBoundary state={dehydratedState}> // [!코드 ++]
  <App />
- </Hydrate> // [!코드-]
+ </HydrationBoundary> // [!코드 ++]
```
### Query 기본값 변경

`queryClient.getQueryDefaults`는 이제 첫 번째 일치하는 등록만 반환하는 대신 일치하는 모든 등록을 함께 병합합니다.

결과적으로 이제 `queryClient.setQueryDefaults`에 대한 호출은 _증가하는_ 특이성으로 정렬되어야 합니다.
즉, 등록은 **가장 일반적인 키**에서 **가장 덜 일반적인 키**까지 이루어져야 합니다.

예를 들어:

```ts
+ queryClient.setQueryDefaults(['todo'], {   // [!코드 ++]
+   retry: false,  // [!코드 ++]
+   staleTime: 60_000,  // [!코드 ++]
+ })  // [!코드 ++]
...
- }) // [!코드-]
```
이 특정 예에서는 `retry: true`가 `['todo', 'detail']` 등록에 추가되어 이제 보다 일반적인 등록에서 `retry: false`를 상속합니다. 정확한 동작을 유지하는 데 필요한 구체적인 변경 사항은 기본값에 따라 달라집니다.
...
```
여기서는 데이터를 캐시에 직접 쓰는 대신 mutation가 실행 중일 때 UI가 표시되는 방식만 변경합니다. 이는 낙관적 업데이트를 표시해야 하는 위치가 한 곳뿐인 경우 가장 잘 작동합니다. 자세한 내용은 [낙관적 업데이트 설명서](optimistic-updates.md)를 참조하세요.
...
Infinite Queries는 일반 Queries처럼 프리패치할 수 있습니다. 기본적으로 Query의 첫 번째 페이지만 프리페치되어 지정된 QueryKey에 저장됩니다. 두 페이지 이상을 프리페치하려면 `pages` 옵션을 사용할 수 있습니다. 자세한 내용은 [프리페칭 가이드](prefetching.md)를 읽어보세요.

### `useQueries`를 위한 새로운 `combine` 옵션

자세한 내용은 [useQueries](../reference/useQueries.md#combine)를 참조하세요.

### 실험적 `fine grained storage persister`

자세한 내용은 [experimental_createQueryPersister](../plugins/createPersister.md)를 참조하세요.

[//]: # 'FrameworkSpecificNewFeatures'

### Query 옵션을 생성하는 안전한 방법

자세한 내용은 [TypeScript](../typescript.md#typing-query-options)를 참조하세요.

### suspense를 위한 새로운 후크

v5를 사용하면 데이터 가져오기용 suspense가 마침내 "안정화"됩니다. 전용 `useSuspenseQuery`, `useSuspenseInfiniteQuery` 및 `useSuspenseQueries` 후크를 추가했습니다. 이러한 후크를 사용하면 `data`는 유형 수준에서 잠재적으로 `undefined`가 될 수 없습니다.
...
```
query 후크의 실험적 `suspense: boolean` 플래그가 제거되었습니다.

이에 대한 자세한 내용은 [Suspense](suspense.md)에서 확인할 수 있습니다.
