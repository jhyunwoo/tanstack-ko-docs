...
const usersMessages = useQueries({
  queries: userIds
    ...
    : [], // userIds가 정의되지 않은 경우 빈 배열이 반환됩니다.
})
```
[//]: # 'Example2'
...
정의에 따라 종속 queries는 성능을 저하시키는 [요청 폭포](request-waterfalls.md) 형식을 구성합니다. 두 queries 모두 동일한 시간이 걸리는 것으로 가정하면 병렬이 아닌 직렬로 수행하면 항상 두 배의 시간이 소요됩니다. 이는 대기 시간이 긴 클라이언트에서 발생할 때 특히 해롭습니다. 가능하다면 두 queries를 병렬로 가져올 수 있도록 백엔드 API를 재구성하는 것이 항상 더 좋지만 실제로는 항상 가능하지는 않습니다.
