...
애플리케이션 성능은 광범위하고 복잡한 영역이며 React Query는 API 속도를 더 빠르게 만들 수 없지만 최고의 성능을 보장하기 위해 React Query를 사용하는 방법에 유의해야 할 사항이 여전히 있습니다.

React Query 또는 실제로 구성 요소 내부에서 데이터를 가져올 수 있는 데이터 가져오기 라이브러리를 사용할 때 가장 큰 성능 기반은 요청 폭포입니다. 이 페이지의 나머지 부분에서는 이러한 현상이 무엇인지, 이를 발견하는 방법, 이를 방지하기 위해 애플리케이션이나 API를 재구성하는 방법에 대해 설명합니다.

[프리페칭 및 라우터 통합 가이드](prefetching.md)는 이를 기반으로 하며 애플리케이션이나 API를 재구성하는 것이 불가능하거나 가능하지 않을 때 미리 데이터를 프리페치하는 방법을 알려줍니다.

[서버 렌더링 및 Hydration 가이드](ssr.md)에서는 서버에서 데이터를 미리 가져오고 해당 데이터를 클라이언트에 전달하여 다시 가져올 필요가 없도록 하는 방법을 알려줍니다.

[//]: # 'AdvancedSSRLink'

[고급 서버 렌더링 가이드](advanced-ssr.md)에서는 이러한 패턴을 서버 구성 요소 및 스트리밍 서버 렌더링에 적용하는 방법을 자세히 설명합니다.
...
## 폭포수 요청 및 React Query

이제 React Query를 고려해 보겠습니다. 먼저 서버 렌더링이 없는 경우에 중점을 둘 것입니다. query 만들기를 시작하기 전에 JS를 로드해야 하므로 해당 데이터를 화면에 표시하기 전에 이중 폭포가 있습니다.
...
3.     |-> Query
```
이를 토대로 React Query에서 요청 폭포수로 이어질 수 있는 몇 가지 다른 패턴과 이를 방지하는 방법을 살펴보겠습니다.
...
단일 구성 요소가 먼저 하나의 query를 가져온 다음 다른 구성 요소를 가져오는 경우 이는 요청 폭포입니다. 이는 두 번째 query가 [종속 Query](dependent-queries.md)일 때 발생할 수 있습니다. 즉, 가져올 때 첫 번째 query의 데이터에 따라 다릅니다.
...
> API를 재구성하지 않고 종속 queries를 완화하는 또 다른 방법은 워터폴을 지연 시간이 더 낮은 서버로 이동하는 것입니다. 이는 [고급 서버 렌더링 가이드](advanced-ssr.md)에서 다루는 서버 구성 요소의 기본 아이디어입니다.

[//]: # 'ServerComponentsNote1'
[//]: # 'SuspenseSerial'

직렬 queries의 또 다른 예는 Suspense와 함께 React Query를 사용하는 경우입니다.
...
이 폭포수를 평면화하는 또 다른 방법은 `<Article>` 구성 요소의 주석을 프리페치하거나 페이지 로드 또는 페이지 탐색 시 라우터 수준에서 이러한 queries를 모두 프리페치하는 것입니다. 이에 대한 자세한 내용은 [프리페치 및 라우터 통합 가이드](prefetching.md)를 참조하세요.
...
이 예에서는 query를 상위 항목으로 끌어올리거나 프리페칭을 추가하는 것만으로는 폭포를 간단하게 평탄화할 수 없습니다. 이 가이드의 시작 부분에 있는 종속 query 예제와 마찬가지로 한 가지 옵션은 `getFeed` query에 그래프 데이터를 포함하도록 API를 리팩터링하는 것입니다. 또 다른 고급 솔루션은 서버 구성 요소를 활용하여 대기 시간이 더 낮은 서버로 워터폴을 이동하는 것입니다([고급 서버 렌더링 가이드](advanced-ssr.md)에서 자세히 알아보기). 그러나 이는 매우 큰 아키텍처 변경이 될 수 있습니다.
...
5.         |> getGraphDataById()
```
이는 서버 렌더링 시 약간 다르게 보이므로 [서버 렌더링 및 Hydration 가이드](ssr.md)에서 자세히 살펴보겠습니다. 또한 `<Feed>`가 포함된 경로가 코드 분할되어 또 다른 홉이 추가되는 경우도 드물지 않습니다.
...
2.   |> JS for <GraphFeedItem>
```
그러나 이것은 매우 절충안입니다. 이제 `<Feed>`와 동일한 번들에 `getGraphDataById`에 대한 데이터 가져오기 코드가 포함되어 있으므로 귀하의 사례에 가장 적합한 것이 무엇인지 평가하십시오. [프리페칭 및 라우터 통합 가이드](prefetching.md)에서 이 작업을 수행하는 방법에 대해 자세히 알아보세요.
...
>
>는 훌륭하지 않으며 서버 구성 요소에 대한 동기 중 하나였습니다. 서버 구성 요소를 사용하면 두 가지를 모두 피할 수 있습니다. [고급 서버 렌더링 가이드](advanced-ssr.md)에서 이것이 React Query에 어떻게 적용되는지 자세히 알아보세요.
...
다음 가이드에서는 [프리페칭 및 라우터 통합](prefetching.md)을 활용하여 워터폴을 평면화하는 더 많은 방법을 살펴보겠습니다.