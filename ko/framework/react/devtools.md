...
React Query에는 전용 개발 도구가 함께 제공되므로 공중에 손을 흔들고 만세를 외쳐보세요! 🥳

React Query 여정을 시작할 때 이러한 개발 도구를 곁에 두고 싶을 것입니다. 이는 React Query의 모든 내부 작동을 시각화하는 데 도움이 되며 위기에 처했을 때 디버깅 시간을 절약해 줄 것입니다!
...
> React Native 사용자의 경우: 모든 js 기반 애플리케이션에서 React Query를 디버깅하는 데 타사 기본 macOS 앱을 사용할 수 있습니다. 실시간으로 여러 장치에서 queries를 모니터링하세요. 여기에서 확인하세요: [rn-better-dev-tools](https://github.com/LovesWorking/rn-better-dev-tools)

> 버전 5부터 개발 도구는 mutations 관찰도 지원합니다.
...
```
Next 13+ App Dir의 경우 작동하려면 개발자 종속성으로 설치해야 합니다.
...
Devtool은 프로덕션 빌드에서 제외됩니다. 그러나 프로덕션 환경에서는 개발 도구를 지연 로드하는 것이 바람직할 수 있습니다.
...
```
TypeScript의 경우 tsconfig에서 `moduleResolution: 'nodenext'`를 설정해야 하며, 이를 위해서는 최소한 TypeScript v4.7이 필요합니다.